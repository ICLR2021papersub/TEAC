...
class MLPDActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, hidden_sizes=(256, 256),
        ...
        # build policy and value functions
        self.pi1 = SquashedGaussianMLPActor(
            obs_dim, act_dim, hidden_sizes, activation, act_limit)
        self.pi2 = SquashedGaussianMLPActor(
            obs_dim, act_dim, hidden_sizes, activation, act_limit)
        self.q1 = MLPQFunction(obs_dim, act_dim, hidden_sizes, activation)
        self.q2 = MLPQFunction(obs_dim, act_dim, hidden_sizes, activation)
    ...
    def act1(self, obs, deterministic=False):
        with torch.no_grad():
            a, _ = self.pi1(obs, deterministic, False)
            return a.cpu().numpy()

    def act2(self, obs, deterministic=False):
        with torch.no_grad():
            a, _ = self.pi2(obs, deterministic, False)
            return a.cpu().numpy()
